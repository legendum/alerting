 ...
 * list surface here. */
export function resolveDeps(requested) {
    const seen = new Set();
    const walk = (p) => {
        if (seen.has(p))
            return;
        ...
        if (!entry) {
            throw new Error(`Unknown pues part "${p}". Known: ${Object.keys(PUES_MANIFEST).join(", ")}.`);
        }
        for (const dep of Object.keys(entry.depends))
            walk(dep);
    };
