 ...
 */
export function onPuesUnauthorized(cb) {
    unauthorizedHandler = cb;
    return () => {
        if (unauthorizedHandler === cb)
...
function shouldFireUnauthorized(url) {
    if (typeof window === "undefined")
        return true;
    if (!url)
        return true;
    if (url.origin !== window.location.origin)
        return false;
    if (url.pathname.startsWith(PUES_AUTH_PREFIX))
        return false;
    return true;
}
/**
 ...
 * unchanged.
 */
export function wrapFetchWithUnauthorized(baseFetch) {
    const wrapped = async (input, init) => {
        const response = await baseFetch(input, init);
        if (response.status === 401 && unauthorizedHandler) {
            const url = resolveRequestUrl(input);
            if (shouldFireUnauthorized(url))
                unauthorizedHandler();
        }
        return response;
    };
