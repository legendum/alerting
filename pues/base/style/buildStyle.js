...
import { readStyleConfig } from "./config";
import { cssVarName, DEFAULT_TOKENS, TOKEN_NAMES } from "./tokens";
export function buildStyle({ root }) {
    const cfg = readStyleConfig(root);
    const defaultsCss = readFileSync(join(import.meta.dirname, "defaults.css"), "utf8");
...
function render(cfg, defaultsCss) {
