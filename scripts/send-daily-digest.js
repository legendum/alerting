...
function parseEmailPolicy(policyJson) {
    if (!policyJson?.trim())
        return "never";
    try {
...
function buildNotificationBoxes(events, timezone) {
    return events
        .map((e) => {
...
const webhooks = db.query("SELECT id, user_id, name, policy FROM webhooks").all();
const dailyByUser = new Map();
for (const w of webhooks) {
    if (parseEmailPolicy(w.policy) !== "daily")
        continue;
 ...
 */
function isMailHourInTimezone(timezone, mailHour) {
...
for (const [userId, { webhookIds, webhookNames }] of dailyByUser) {
    const userRow = db.query("SELECT email, timezone FROM users WHERE id = ?").get(userId);
    if (!userRow?.email)
        continue;
    ...
    const rows = db
        .query(`SELECT e.id, e.webhook_id, e.title, e.body, e.created_at
     FROM webhook_events e
     WHERE e.webhook_id IN (${placeholders}) AND e.created_at >= ?
     ORDER BY e.created_at DESC`)
        .all(...webhookIds, since);
    if (rows.length === 0)
        continue;
