...
const config = Bun.YAML.parse(await Bun.file("config/pues.yaml").text());
...
if (!existsSync(`${DST_BASE}/core/manifest.ts`)) {
    console.error(`Bootstrap copy of base/core did not produce manifest.ts. ` +
...
await Bun.write(`${DST_BASE}/core/puesAppMeta.generated.ts`, `// Auto-generated by scripts/pues.ts from config/pues.yaml — do not edit.\nexport const puesAppMeta = {\n  name: ${JSON.stringify(appName)},\n} as const;\n`);
console.log(`Vendored pues/base: ${parts.join(", ")}` +
    (auto.length > 0 ? ` (auto-pulled: ${auto.join(", ")})` : ""));
if (existsSync(SRC_TYPES_BASE)) {
    console.log(`...types/pues/base: ` +
...
async function syncSkills(sourceDir, targetDir) {
...
async function syncRules(sourceDir, targetDir) {
    if (!existsSync(sourceDir))
        return [];
    await mkdir(targetDir, { recursive: true });
    const synced = [];
