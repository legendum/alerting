...
export async function registerPush(req: Request, userId: number): Promise<Response> {
  ...
  const count = (db.query("SELECT COUNT(*) as n FROM fcm_tokens WHERE user_id = ?").get(userId) as { n: number }).n;
  if (count > maxDevices) {
    ...
    for (const row of oldest) {
      db.run("DELETE FROM fcm_tokens WHERE user_id = ? AND fcm_token = ?", userId, row.fcm_token);
    }
  }
  return json({ ok: true });
}
