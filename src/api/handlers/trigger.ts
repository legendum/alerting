...
export async function triggerWebhook(req: Request, ulidParam: string): Promise<Response> {
  ...
  const webhook = db.query(`
    SELECT w.id, w.token_hash FROM webhooks w
    WHERE w.ulid = ?
  `).get(ulidParam) as { id: number; token_hash: string } | undefined;
  ...
  if (total <= 0) return json({ error: "quota_exceeded", message: "No quota" }, 429);
