...
function withCors(handler) {
  return async (req) => addCors(await handler(req));
}
function wrapCorsRoutes(routes) {
  const out = {};
...
async function withAuth(req, run) {
...
export default {
  ...
  routes: {
    ...
    ...wrapCorsRoutes(webhookResourceRoutes),
  },
  async fetch(req) {
